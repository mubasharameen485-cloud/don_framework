...
async fn dynamic_signup_handler(
    ...
    match query_builder.build().execute(&state.db).await {
        Ok(_) => Ok(Json(AuthResponse { success: true, message: "Account created!".to_string(), token: None, user_data: None })),
...
async fn fixed_login_handler(
    ...
    let (role, final_email, user_metadata) = if payload.email == super_email && payload.password == super_pass {
        ...
        (db_role, payload.email.clone(), metadata)
    };
    ...
    let claims = Claims { sub: final_email, role, exp: expiration };
    let token = encode(&Header::default(), &claims, &EncodingKey::from_secret(secret.as_bytes())).unwrap();

    Ok(Json(AuthResponse { success: true, message: "Login successful!".to_string(), token: Some(token), user_data: user_metadata }))
}