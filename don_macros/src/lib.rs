...
pub fn don_model_derive(input: TokenStream) -> TokenStream {
    ...
    // Update Query Generation (e.g., name=$1, price=$2 WHERE id=$3)
    ...
    let expanded = quote! {
        impl #struct_name {
            ...
            // DATABASE METHODS
            // ==========================================
            pub async fn save(&self, pool: &don_core::sqlx::PgPool) -> Result<Self, don_core::sqlx::Error> {
                don_core::sqlx::query_as::<_, Self>(#insert_query)
                    #(#binds)*
                    .fetch_one(pool)
                    .await
            }

            pub async fn update(&self, pool: &don_core::sqlx::PgPool, id: i32) -> Result<Self, don_core::sqlx::Error> {
                don_core::sqlx::query_as::<_, Self>(#update_query)
                    #(#update_binds)*
                    .bind(id)
                    .fetch_one(pool)
                    .await
            ...
            pub async fn find_all(pool: &don_core::sqlx::PgPool) -> Result<Vec<Self>, don_core::sqlx::Error> {
                don_core::sqlx::query_as::<_, Self>(#select_all_query)
                    .fetch_all(pool)
                    .await
            }

            pub async fn find_by_id(pool: &don_core::sqlx::PgPool, id: i32) -> Result<Self, don_core::sqlx::Error> {
            ...
            pub async fn delete(pool: &don_core::sqlx::PgPool, id: i32) -> Result<u64, don_core::sqlx::Error> {
                let result = don_core::sqlx::query(#delete_query).bind(id).execute(pool).await?;
                Ok(result.rows_affected())
            }

            // ==========================================
            // AXUM API HANDLERS (Auto-Generated)
            // ==========================================
            pub async fn api_create(
                don_core::axum::extract::State(state): don_core::axum::extract::State<don_core::server::AppState>,
                don_core::axum::Json(payload): don_core::axum::Json<Self>,
                ...
                match payload.save(&state.db).await {
                    ...
                    Err(e) => Err((don_core::axum::http::StatusCode::INTERNAL_SERVER_ERROR, e.to_string())),
                }
            }
            ...
            pub async fn api_get_one(
                ...
                match Self::find_by_id(&state.db, id).await {
                    ...
                    Err(_) => Err((don_core::axum::http::StatusCode::NOT_FOUND, "Record not found".to_string())),
                }
            }
